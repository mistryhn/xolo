...
function App() {
  ...
  async function enterZone(coords: { latitude: number; longitude: number }) {
    ...
    setZone(json.data.zoneKey)
    socket.emit('zone:join', coords, () => {})
    const rooms = await fetch(`${api}/api/chats`, {
  ...
  async function create(type: 'private' | 'group') {
    ...
    const r = await fetch(`${api}/api/chats`, {
      ...
      body: JSON.stringify({ type, title, zoneKey }),
    })
    const json = await r.json()
    if (r.ok) {
      setChats((items) => [json.data, ...items])
      join(json.data.id)
    }
  }
  ...
  if (!session)
    return (
      <main>
        <h1>Xolo</h1>
        <p>Real-time conversations in your local zone.</p>
        <form onSubmit={auth}>
          {mode === 'register' && (
            <input
              placeholder="Name"
  ...
  return (
    <main>
      ...
      <p>Local zone: {zoneKey}</p>
      <div className="actions">
        <button onClick={() => create('private')}>Start private match</button>
        <button onClick={() => create('group')}>Create group room</button>
      </div>
      <aside>
        <h2>Open chats</h2>
        {chats.map((chat) => (
          <button className="room" key={chat.id} onClick={() => join(chat.id)}>
            {chat.title ?? 'Private match'} · {chat.type}
          </button>
        ))}
      </aside>
      <ChatWindow />
