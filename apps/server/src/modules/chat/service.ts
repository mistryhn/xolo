...
import type { CreateChatInput } from '@xolo/protocol'
const words = ['Azure', 'Brisk', 'Calm', 'Dawn', 'Ember']
export const aliasFor = () =>
  `${words[Math.floor(Math.random() * words.length)]}-${Math.floor(1000 + Math.random() * 9000)}`
export async function createChat(userId: string, zoneKey: string, input: CreateChatInput) {
  const [chat] = await db
    .insert(chats)
    .values({
      creatorId: userId,
      ...
      type: input.type,
      title: input.title,
      memberCap: input.type === 'private' ? 2 : (input.memberCap ?? 50),
    })
    .returning()
  await db.insert(chatParticipants).values({ chatId: chat.id, userId, alias: aliasFor() })
  return chat
}
...
export async function joinChat(chatId: string, userId: string) {
  ...
  const [member] = await db
    ...
    .values({ chatId, userId, alias: aliasFor() })
    .returning()
